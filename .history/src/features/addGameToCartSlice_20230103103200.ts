...
export const addGameToCart = createSlice({
  ...
  reducers: {
    ...
    incrementCountGame (state, action) {
      state.gameInCart = state.gameInCart.map(game => game.count += action.payload)
    } 
  },
});

export const { addGame, removeGame, deleteAllFromCart, incrementCountGame } = addGameToCart.actions;
