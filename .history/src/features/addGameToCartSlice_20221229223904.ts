...
export const addGameToCart = createSlice({
  ...
  reducers: {
    ...
    incrGameCount(state, action: PayloadAction<number> ) {
      state.gameInCart.map(item => item.count += 1);
    }
  },
});

export const { addGame, removeGame, deleteAllFromCart, incrGameCount } = addGameToCart.actions;
