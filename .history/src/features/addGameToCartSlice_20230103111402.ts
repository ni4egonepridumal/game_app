...
export const addGameToCart = createSlice({
  ...
  reducers: {
    ...
    totalSummGameInCart(state, action) {
      state.gameInCart.reduce((acc, game) => (acc += game.price*action.payload), 0);
    },
  },
});

export const { addGame, removeGame, deleteAllFromCart, totalSummGameInCart } = addGameToCart.actions;
