...
export const CartPage: React.FC = () => {
  ...
  const totall = useAppSelector((state) => state.addGameToCart);
  console.log(totall.gameInCart.map(item=> item.count))
  ...
  return (
    <>
      <h2>Корзина</h2>
      {gameInCart.length > 0 ? (
        gameInCart.map((item) => <GameInCart  some={some} key={uuidv4()} game={item} />)
      ) : (
