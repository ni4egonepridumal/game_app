...
export const CartPage: React.FC = () => {
  ...
  const [countGame, setCountGame] = React.useState<number>(1);
  ...
  return (
    <>
      <h2>Корзина</h2>
      {gameInCart.length > 0 ? (
        gameInCart.map((item) => <GameInCart setCountGame={setCountGame} countGame={countGame} key={uuidv4()} game={item} />)
      ) : (
      ...
      {gameInCart.length > 0 ? (
        <div className={styles.cartPageBottom}>
          <div>Всего на сумму: {countGame*totalPrice}</div>
